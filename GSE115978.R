...
# 载入GSE115978的数据
# exprs_GSE115978 <- readRDS("dataz/exprs_GSE115978.rds")
# pd_GSE15978 <- readRDS("dataz/pd_GSE115978.rds") %>% tibble::column_to_rownames("Cell")
# sc_GSE115978 <- CreateSeuratObject(exprs_GSE115978, min.cells=1)
# sc_GSE115978 <- AddMetaData(sc_GSE115978, metadata = pd_GSE15978)
# sc_GSE115978 <- NormalizeData(sc_GSE115978)
# sc_GSE115978 <- FindVariableFeatures(sc_GSE115978, selection.method = "vst", nfeatures = 2000)
# sc_GSE115978 <- ScaleData(sc_GSE115978)
# sc_GSE115978 <- RunPCA(sc_GSE115978)
# sc_GSE115978 <- RunUMAP(sc_GSE115978, dims = 1:10)
sc_GSE115978 <- readRDS("sc_GSE115978.rds")
Idents(sc_GSE115978) <- 'cellType'

#绘制umap图
A <- DimPlot(sc_GSE115978, reduction = "umap", label = F, label.size = 4) + 
  NoLegend() +
  ggtitle('SKCM GSE115978') + 
...
sc_GSE115978 <- AddModuleScore(sc_GSE115978, features = markers, name = names(markers), seed = 123)
colnames(sc_GSE115978@meta.data)[7:ncol(sc_GSE115978@meta.data)] <- names(markers)
mydata <- FetchData(sc_GSE115978,vars = c("Response", names(markers), "UMAP_1", "UMAP_2", "cellType"))
...
C <- ggboxplot(box_data, x="Response", y="value", fill  = "Response",
               palette = good9color, width=0.6, add = "none")+
  xlab("")+
...
# p_m
# ggsave("Figure2.pdf", plot = p_m, width = 17, height = 20, units = "cm")

# saveRDS(sc_GSE115978, file = "dataz/figure2_input.rds")